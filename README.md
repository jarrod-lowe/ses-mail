...
1. Upload your Gmail token and OAuth client credentials to SSM Parameter Store:

   **Gmail OAuth Token** (from token.json):
   ...
   **Gmail OAuth Client Credentials** (from client_secret.json):
   ...
   The OAuth client credentials are used by the enhanced refresh script to obtain new refresh tokens when needed. Upload the complete contents of your `client_secret.json` file:
   ...
   **Important**: The client credentials and OAuth tokens are different:
   - **Client Credentials** (`client_secret.json`): OAuth application credentials from Google Cloud Console. Used to obtain new refresh tokens. Contains `client_id`, `client_secret`, and `redirect_uris`.
   - **OAuth Token** (`token.json`): Contains the refresh token and access token for Gmail API access. Generated by running the OAuth flow with client credentials.
...
1. **OAuth Client Credentials** (from Google Cloud Console):
   ...
   - **Purpose**: Used by the refresh script to initiate OAuth flows and obtain new refresh tokens
   - **Lifetime**: No expiration (unless revoked in Google Cloud Console)

2. **OAuth Token** (generated by OAuth flow):
   - **Parameter Path**: `/ses-mail/{environment}/gmail-token`
   - **Content**: Token JSON containing refresh token and access token
   - **Purpose**: Used by Gmail forwarder Lambda to access Gmail API
   - **Lifetime**:
     - **Refresh Token**: 7 days in Google OAuth testing mode (indefinite in production mode)
     - **Access Token**: ~1 hour (automatically refreshed by Lambda using refresh token)
